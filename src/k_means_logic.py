...
def run_kmeans(data: np.ndarray, original_labels: np.ndarray, n_clusters: int = 3, 
               random_state: int = 42) -> Dict[str, Any]:
    """
    Executes K-Means, remaps cluster labels to match original groups, 
    ...
    # --- Remap cluster labels to match original groups ---
    # Create a cost matrix (confusion matrix)
    cm = confusion_matrix(original_labels, raw_labels)
    
    # Use the Hungarian algorithm to find the best assignment
    # We maximize the overlap, so we use the negative of the confusion matrix
    row_ind, col_ind = linear_sum_assignment(-cm)
    
    # Create the mapping and remap the labels
    mapping = {col: row for row, col in zip(row_ind, col_ind)}
    labels = np.array([mapping.get(label, -1) for label in raw_labels])
    
    # Reorder centroids based on the mapping
    centroids = np.zeros_like(kmeans.cluster_centers_)
    for k_from, k_to in mapping.items():
        centroids[k_to] = kmeans.cluster_centers_[k_from]
    ...
    for k in range(n_clusters):
        ...
        if len(cluster_points) > 0:
            ...
            distance_sq = np.sum((centroids[k] - global_mean)**2)
